...
public class TrackingFilter implements GlobalFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		if (isCorrelationIdPresent(requestHeaders)) {
			logger.debug("gtw-correlation-id exist in tracking filter: {}. ",
					filterUtils.getCorrelationId(requestHeaders));
		} else {
			String correlationID = generateCorrelationId();
			exchange = filterUtils.setCorrelationId(exchange, correlationID);
			logger.debug("gtw-correlation-id is created in tracking filter: {}.", correlationID);
		}

		System.out.println("The authentication name from the parsed json token is : " + getUsername(requestHeaders));
